...
struct MockSession : session {
  MockSession(boost::asio::io_service& io): session(io) {}
...
struct TempSession : MockSession {
  TempSession(boost::asio::io_service& io): MockSession(io) {}
...
protected:
  ...
  void SetUp() override {
    mock_sess_ = new StrictMock<MockSession>(io_);
    srv_ = std::make_unique<server>(io_, /*port=*/5555);
  }
...
TEST_F(ServerTest, HandleAccept_CalledMultipleTimes) {
  ...
  StrictMock<MockSession> second_mock(io_);
