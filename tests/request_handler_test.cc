...
protected:
    ...
    void SetUp() override {
        ...
        ASSERT_FALSE(ec) << "Failed to create temporary directory: " << temp_dir_.string() << " Error: " << ec.message();
        ...
        try {
         ...
         ASSERT_TRUE(static_handler_ != nullptr) << "static_handler_ pointer is null after instantiation.";
    ...
    void TearDown() override {
        ...
        if (ec) {
            std::cerr << "Warning: Failed to remove temp dir: " << temp_dir_.string() << " Error: " << ec.message() << std::endl;
        }
    }

    void CreateTestFile(const std::string& filename, const std::string& content) {
        std::ofstream ofs(temp_dir_ / filename);
        ASSERT_TRUE(ofs.is_open()) << "Failed to open file for writing: " << (temp_dir_ / filename).string();
        ofs << content;
...
TEST_F(StaticHandlerIntegrationTest, PathTraversalBlockedSimple) {
    ...
    http::request<http::string_body> req = create_request(http::verb::get, "/static/../some_other_file");
    http::response<http::string_body> res;
...
TEST_F(StaticHandlerIntegrationTest, ServeFileWithEncodedSpaces) {
    ...
    http::request<http::string_body> req = create_request(http::verb::get, "/static/file%20with%20spaces.txt");
    http::response<http::string_body> res;
...
// Often this might serve an index.html file if present, or return 404/403 if directory listing is disabled.
// Assuming here it should fail if index.html doesn't exist.
TEST_F(StaticHandlerIntegrationTest, RequestStaticRootWithoutIndex) {
     // Arrange
     http::request<http::string_body> req = create_request(http::verb::get, "/static/"); // Request the root
     http::response<http::string_body> res;
