...
void logger::init(const std::string &pattern) {
  ...
  logging::add_file_log(
      logging::keywords::file_name = pattern, logging::keywords::rotation_size = 10 * 1024 * 1024,
      logging::keywords::time_based_rotation = sinks::file::rotation_at_time_point{0, 0, 0},
      logging::keywords::open_mode = std::ios_base::app, logging::keywords::auto_flush = true,
      logging::keywords::format =
          expr::stream << "["
                       << expr::format_date_time<boost::posix_time::ptime>("TimeStamp",
                                                                           "%Y-%m-%d %H:%M:%S.%f")
