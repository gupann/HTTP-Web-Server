...
std::string url_decode_simple(const std::string& encoded) {
  ...
  for (size_t i = 0; i < encoded.length(); ++i) {
      if (encoded[i] == '%' && i + 2 < encoded.length() && encoded[i+1] == '2' && encoded[i+2] == '0') {
          decoded += ' ';
          i += 2;
      } else if (encoded[i] == '+') {
           decoded += ' ';
...
void static_handler::handle_request(http::request<http::string_body>& req, http::response<http::string_body>& res) {
  ...
  if (decoded_target.rfind(url_prefix, 0) != 0) {
      ...
      BOOST_LOG_TRIVIAL(error) << "Static handler called for wrong prefix. Target: " << decoded_target << ", Prefix: " << url_prefix;
  ...
  if (file_path.find("..") != std::string::npos || file_path.rfind(root_dir, 0) != 0) {
      BOOST_LOG_TRIVIAL(warning) << "Potential directory traversal blocked for path: " << file_path << " (Relative: " << relative_path << ")";
      res.result(http::status::not_found);
      res.set(http::field::content_type, "text/plain");
      res.version(req.version());
